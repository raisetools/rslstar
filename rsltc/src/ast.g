...
'export'
    ...
    Transition_rule_collection_id New_transition_rule_collection_id
    OPT_TRANS_RULE_ID
    ...
    -- Genericity unfolding
    STRINGS STRINGS_LIST
    SUBSTITUTE SUBSTITUTES SUBSTITUTES_LIST
...
'type' TYPE_EXPR
	...
	poly		(num	:	INT)
        array           (dom_type:	TYPE_EXPR,
                         rng_type:	TYPE_EXPR)
    generic     (type   :   TYPE_EXPR,
                 for_types : TYPE_EXPRS_GEN)
...
'table' Transition_rule_collection_id (Pos : POS,
            Ident   : IDENT,
            Body    : TRANSITION_OPERATOR)

'action' New_transition_rule_collection_id(POS, IDENT, TRANSITION_OPERATOR -> Transition_rule_collection_id)

  'rule' New_transition_rule_collection_id(P, I, OP -> New):
        New::Transition_rule_collection_id
        New'Pos <- P
        New'Ident <- I
        New'Body <- OP
...
'type' OPT_TRANS_RULE_ID

       tr_id (Transition_rule_collection_id)
...
'type' TRANS_RULE_ENV
       ...
       trans_rule_env	(Transition_rule_collection_id,
			 TRANS_RULE_ENV)
       nil
...
'type' SYSTEM_DESCRIPTION
       ...
       with_constraint(LocalVarDecls	:	DECL,		-- here too!
         InitConstraint : CONSTRAINT, -- boolean expression
		 Transition	:	TRANSITION_OPERATOR)

       as_relation(LocalVarDecls	:	DECL,		-- here too!
         InitConstraint : CONSTRAINT, -- boolean expression
         TransitionRelation : TRANSREL) -- boolean expression
...
'type' CONSTRAINT 
	...
	constraint 	(pos	: 	POS,
        conjs :	CONJUNCTION_LIST)

'type' CONJUNCTION_LIST

	list	(head	:	VALUE_EXPR,
		tail	:	CONJUNCTION_LIST)
...
'type' TRANSITION_OPERATOR
    ...
    trans_naming(operator : TRANSITION_OPERATOR,
            namedlist : NAMED_TRANS_LIST)

    trans_ident (id : IDENT)
...
'type' NAMED_TRANS_LIST

        list(head : NAMED_TRANS,
            rest : NAMED_TRANS_LIST)

        nil

'type' NAMED_TRANS

        named_trans(name : IDENT,
            trans : TRANSITION_OPERATOR)
...
'type' SUBSTITUTES
        ...
        list( head  :   SUBSTITUTE,
            rest : SUBSTITUTES)

        nil

'type' SUBSTITUTES_LIST

        list( head  :   SUBSTITUTES,
            rest : SUBSTITUTES_LIST)
