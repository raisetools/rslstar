  ...
  'rule' Resolve_value_def(exp_fun(P, single(_,single(P1,F),_), form_appl(_,_,PARMS), V, POST, PRE, _)):
	 ...
	 (|
	   ...
	   (|
	     where(RP -> result(_, B))
	     Openenv()
	     Define_value_binding(B, T2, no_def)
             Resolve(EP, bool -> EP1)
	     Closeenv()
	   ||
	     Resolve(EP, bool -> EP1)
	 ...
	 (|
	   ...
	   Simplify(COND1 -> COND2)
	   (|
	     (| where(COND2 -> literal_expr(_,bool_true)) || where(COND2 -> no_val) |)
	     where(OPT_CONDITION'nil -> COND)
	   ||
	     where(OPT_CONDITION'condition(COND2) -> COND)
	   |)
	   ...
	   (|
	     (| where(PCOND2 -> literal_expr(_,bool_true)) || where(PCOND2 -> no_val) |)
...
/*         (|
  ...
  'rule' Resolve_type(subtype(TP, restriction(P, E)) -> subtype(TP1, restriction(P, E1))):
	 ...
	 Openenv()
	 Define_value_typing(TP1, no_def)
	 Resolve(E, bool -> E1)
	 [|
	   CPPWanted()
	   Localise_value_env()
...
-- useful for translation to know type
 'rule' Resolve(enum_list(P, nil), T -> V):
  ...
  'rule' Resolve(comp_list(P, V, list_limit(P1, B, V1, R)), T -> comp_list(P, V0, list_limit(P1, B, V11, R1))):
	 ...
	 Openenv()
	 Define_value_typing(single(P1, B, T2),no_def)
	 (|
	   where(R -> restriction(P2,V2))
	   Resolve(V2, bool -> V3)
	   where(restriction(P2,V3) -> R1)
...
-- useful for translation to know type
  ...
  'rule' Resolve(post(P, V, post_cond(P1, R, C), PRE), T -> post(P, V1, post_cond(P1, R, C1), PRE1)):
	 ...
	 (|
	   where(R -> result(P3, B))
	   Openenv()
	   Define_value_binding(B, T1, no_def)
	   Resolve(C, bool -> C1)
	   Closeenv()
	 ||
	   Resolve(C, bool -> C1)
...
--	   I'Qualifier -> Ids
  ...
  'rule' Resolve(class_scope_expr(P, CL:instantiation(name(_,id_op(Id)), nil), V), T -> env_class_scope(P, CL, CE, V1)):
	 ...
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,WITH,nil), current_env(CE, C))
  ...
  'rule' Resolve(class_scope_expr(P, CL, V), T -> env_class_scope(P, CL, CE, V1)): 
	 ...
	 Resolve_class(CL)
	 Current -> current_env(CE, C1)
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,WITH,nil), current_env(CE, C1))
	 Extend_paths -> Paths1
...
'action' Resolve_let_def(LET_DEF -> LET_DEF)
  ...
  'rule' Resolve_let_def(explicit(P, LB, V) -> explicit(P, LB1, V2)):
	 ...
	 (|
	   ...
	   Define_value_typing(single(P, B, T1), no_def)
	   where(LB -> LB1)
	 ||
...
'action' Make_type_fits1(CLASS_ENV, ADAPTS -> TYPE_FITS)
  ...
  'rule' Make_type_fits1(basic_env(TYP, list(VAL,VES), VAR, CH, MOD, AX, TC,_, _, _,WITH, AD), AD1 -> TYPF):
	 Concat_adapts(AD1, AD -> AD2)
...
'action' Make_imp_fit1(CLASS_ENV, ADAPTS, TYPE_FITS, OBJECT_FITS -> IMP_FIT)
  ...
  'rule' Make_imp_fit1(basic_env(TYP, list(VAL,VES), VAR, CH, MOD, AX, TC, _, _, _, WITH, AD), AD1, TYPF, OBJF -> IF):
	 Concat_adapts(AD1, AD -> AD2)
