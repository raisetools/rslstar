...
-- Check types
   ...
   Match Match_up Unify_by_result Unify_to_resolve
   ...
   Qualify_by_with
   Remove_Brackets
   Check_types_from_typing_defined
   Check_types_from_typing_finite
   Unify_typing_with
...
'action' Lookup_env_type_name(NAME, CLASS_ENV, OWNENV, OWNCLASS  -> OPT_TYPE_ID)
  ...
  'rule' Lookup_env_type_name(N, CE, Ownenv, Ownclass -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(TE,_,_,_,_,_,_,_,_,_,Objs,_))
...
'action' Lookup_env_type_name2(IDENT, CLASS_ENV  -> OPT_TYPE_ID)
  ...
  'rule' Lookup_env_type_name2(Id, CE -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(TE,_,_,_,_,_,_,_,_,_,_,_))
	     Lookup_env(Id1, TE -> I)
	   ||
	     where(CE -> extend_env(CE1, CE2, _, _))
...
-- Putmsg(" checked\n")
  ...
  'rule' Define_type(T -> T): /* TODO: Add handling for arrays and generic variables? When declaring a type name for an array type, it is not checked that the index and element types exist */
...
'action' Qualify_class_env(Object_id, CLASS_ENV -> CLASS_ENV)
  ...
  'rule' Qualify_class_env(I, basic_env(TYP, VAL, VAR, CH, MOD, AX, TC, TS, TR, Prop, WITH, AD) ->
				basic_env(TYP1, VAL1, VAR1, CH1, MOD1, AX, TC, TS, TR, Prop, WITH, AD)):  
...
-------
'action' Check_types_from_typing_defined(TYPINGS)
  'rule' Check_types_from_typing_defined(list(H,Tail)):
         Check_type_from_typing_defined(H)
         Check_types_from_typing_defined(Tail)

  'rule' Check_types_from_typing_defined(nil):

'action' Check_type_from_typing_defined(TYPING)
  'rule' Check_type_from_typing_defined(single(P,B,T)):
         Check_type_defined(T -> T1)

  'rule' Check_type_from_typing_defined(multiple(P,B,T)):
         Check_type_defined(T -> T1)

'action' Check_types_from_typing_finite(TYPINGS)
  'rule' Check_types_from_typing_finite(list(H,Tail)):
         Check_type_from_typing_finite(H)
         Check_types_from_typing_finite(Tail)

  'rule' Check_types_from_typing_finite(nil):

'action' Check_type_from_typing_finite(TYPING)
  'rule' Check_type_from_typing_finite(single(P,B,T)):
         Check_type_finite(P,T)

  'rule' Check_type_from_typing_finite(multiple(P,B,T)):
         Check_type_finite(P,T)

/* Strictly checks that the type is either a variant enumeration type or 
   a subtype of some type which can be unified with Int and which is restricted 
   such that it is finite -- either by membership of a finite set or by an interval, 
   i.e., less than some number and greater than some number. These are the only 
   forms allowed for generic variables. */
'action' Check_type_finite(POS,TYPE_EXPR)
  'rule' Check_type_finite(P, subtype(Typing,Restriction)):
         (|
           Unify_typing_with(Typing, TYPE_EXPR'int)
           Check_int_restriction_finite(Restriction)
         ||
           Unify_typing_with(Typing, TYPE_EXPR'bool)
           Check_bool_restriction_finite(Restriction)
         ||
           Puterror(P) 
           Putmsg("Generic variables can only be declared over subtypes with maximal type Int or Bool.")
           Putnl()
         |)

  'rule' Check_type_finite(P, named_type(Name)): 
         ...
         [|
           ...
           (|
             ...
             Check_variant_enum(Vs)
           ||
             Puterror(P) 
             Putmsg("Generic variables can only be declared over variant types enumerating constants.")
             Putnl()           
           |)
         |]

  'rule' Check_type_finite(P, _):
         Puterror(P) 
         Putmsg("Generic variables can only be declared over finite types.")
         Putnl()

'condition' Check_variant_enum(VARIANTS)
  'rule' Check_variant_enum(list(H,Tail)):
           where(H -> record(_,C,Cmps))
           [|
            where(C -> constructor(_,Id_or_op))
            where(Id_or_op -> id_op(Ident))
            Check_variant_components(Cmps)
           |]
           Check_variant_enum(Tail)

  'rule' Check_variant_enum(nil):

'condition' Check_variant_components(COMPONENTS)
  'rule' Check_variant_components(list(H,Tail)):
         where(H -> component(_,Dest,Type,Recon))
         where(Dest -> nil)
         where(Recon -> nil)
         where(Type -> unit)
         Check_variant_components(Tail)
        
  'rule' Check_variant_components(nil):
        

'condition' Unify_typing_with(TYPING, TYPE_EXPR)
  'rule' Unify_typing_with(single(_, _, T),TE):
         Match(TE, down, T)
         
  'rule' Unify_typing_with(multiple(_,_,T),TE):
         Match(TE, down, T)

'action' Check_int_restriction_finite(RESTRICTION)
  'rule' Check_int_restriction_finite(restriction(P,VE)):
         (|
           where(VE -> val_infix(P1,VEL,OP,VER))
           (|
             where(VER -> ranged_set(P2,VEL1,VER1))
           ||
             where(VER -> enum_set(P2,VE1))
           |) 
         ||
         ...
         ||
           Puterror(P)
           Putmsg("Restriction of subtype must result in a finite set of possible values, so it must be either membership of ranged set, membership of enumeration set or an expression of the form x > num1 /\ x < num2.")
           Putnl()
         |)

'action' Check_bool_restriction_finite(RESTRICTION)
  'rule' Check_bool_restriction_finite(restriction(P,VE)):
         (|
           where(VE -> val_infix(P1,VEL,OP,VER))
           where(VER -> enum_set(P2,VE1))
         ||
           Puterror(P) 
           Putmsg("Generic variables can only be declared over subtypes with maximal type Bool if the restriction is membership of a set enumeration.")
           Putnl()
         |)
...
-- |]
  ...
  'rule' Check_type_defined(generic(T,TS) -> generic(T1,TS1)):
         Check_type_defined(T -> T1)
         Check_types_defined(TS -> TS1)         

  'rule' Check_type_defined(T -> T):

'action' Check_types_defined(TYPE_EXPRS_GEN -> TYPE_EXPRS_GEN)
  'rule' Check_types_defined(type_exprs(list(H,Tail)) -> type_exprs(list(H1,Tail1))):
         Check_type_defined(H -> H1)
         Check_types_defined(type_exprs(Tail) -> type_exprs(Tail1))

  'rule' Check_types_defined(type_exprs(nil) -> type_exprs(nil)):
...
-- Print_substitution(S2)
  ...
  /* This was changed to use wildcard (_) instead of nil as the last output param.
     Otherwise resulted in problems when initialising arrays in the 
     initialisation_constraint section. */
  'rule' Unify(array(TI1,TV1), Dir1, array(TI2,TV2), Dir2, S -> array(TR1,TR2), S):
         --Check_type(TI1,TI2)
         Unify(TI1, Dir1, TI2, Dir2, S -> TR1, _)
         Unify(TV1, Dir1, TV2, Dir2, S -> TR2, _)
             
  'rule' Unify(generic(T1,TS1), Dir1, generic(T2,TS2), Dir2, S -> generic(T,TS), S):
	 Unify(T1, Dir1, T2, Dir2, S -> T, _)
     Unify_types(TS1, Dir1, TS2, Dir2, S -> TS, _)
...
'action' Unify_types(TYPE_EXPRS_GEN, DIRECTION, TYPE_EXPRS_GEN, DIRECTION, SUBSTITUTION -> TYPE_EXPRS_GEN, SUBSTITUTION)

  'rule' Unify_types(type_exprs(list(T1,TS1)), Dir1, type_exprs(list(T2, TS2)), Dir2, S -> type_exprs(list(T, TS)), S2):
	 Unify(T1, Dir1, T2, Dir2, S -> T, S1)
	 Unify_types(type_exprs(TS1), Dir1, type_exprs(TS2), Dir2, S1 -> type_exprs(TS), S2)

  'rule' Unify_types(type_exprs(nil), Dir1, type_exprs(nil), Dir2, S -> type_exprs(nil), S)

  'rule' Unify_types(any, Dir1, T, Dir2, S -> T2, S):
	 Lookup_sub(0, S -> T1, Dir)
	 (| eq(T1, none) where(type_exprs(list(TYPE_EXPR'none,nil)) -> T2) || where(T -> T2) |)

  'rule' Unify_types(T, Dir1, any, Dir2, S -> T2, S):
	 Lookup_sub(0, S -> T1, Dir)
	 (| eq(T1, none) where(type_exprs(list(TYPE_EXPR'none,nil)) -> T2) || where(T -> T2) |)
