...
--	 Make_variable_env_defs Make_channel_env_defs
	 ...
	 Lookup_variable_name Lookup_current_variable_name Lookup_variable_id
...
'action' Define_value_def(VALUE_DEF)
  ...
  /* Only setting def for explicit functions. */
  'rule' Define_value_def(exp_fun(P,T,A,E,POST,PRE,_)):
     where(A -> form_appl(_,_,Pa))
     where(expl_fun(Pa,E,POST,PRE,nil,nil) -> VD)
	 Define_value_typing(T,VD)

  'rule' Define_value_def(imp_fun(P,T,A,C,PRE)):
	 Define_value_typing(T,no_def)

  'rule' Define_value_def(gen_val(P,Id,TS,TE)):
     Check_type_defined(TE -> T1)
     Get_type_exprs_from_typings(TS -> TX)
     /* Create what corresponds to the typing for a standard value def. 
        Using the name (Id) and the Type (TE) of the generic value. */
     Define_value_typing(single(P,single(P,id_op(Id)),generic(T1,TX)),no_def)


'action' Define_value_typing(TYPING,VALUE_DEFINITION)

  'rule' Define_value_typing(single(P,B,T),VD):
	 Define_value_binding(B,T,VD)

  'rule' Define_value_typing(multiple(P,BS,T),VD):
	 Define_value_mult_bindings(BS,T,VD)
...
'action' Define_value_mult_bindings(BINDINGS, TYPE_EXPR, VALUE_DEFINITION)

  'rule' Define_value_mult_bindings(list(B,BS), T, VD):
	 Check_type_defined(T -> T1)
	 Define_value_binding2(B,T1,VD)
	 Define_value_mult_bindings(BS,T1,VD)

  'rule' Define_value_mult_bindings(nil, T, VD):

'action' Define_value_binding(BINDING, TYPE_EXPR, VALUE_DEFINITION)

  'rule' Define_value_binding(B, T, VD)
	 Check_type_defined(T -> T1)
	 Define_value_binding2(B, T1, VD)

'action' Define_value_binding2(BINDING, TYPE_EXPR, VALUE_DEFINITION)

  'rule' Define_value_binding2(single(P,Id_or_op), T, VD):
	 Define_id_or_op(P, Id_or_op, T, VD -> _)

  'rule' Define_value_binding2(product(P,BS), T, VD):
	 Define_value_bindings(P, BS, T, VD)

'action' Define_value_bindings(POS, BINDINGS, TYPE_EXPR, VALUE_DEFINITION)

  'rule' Define_value_bindings(P, list(H,nil), T, VD):
	 Define_value_binding2(H,T,VD)

  'rule' Define_value_bindings(P, BS, T, VD)
	 Length_bs(BS -> N1)
	 ...
	 (|
	   ...
	   Length_pr(PR -> N2)
	   eq(N1,N2)
	   Define_value_bindings_prod(BS, PR, VD)
	 ||
	   Puterror(P)
	   ...
	   Print_type(T)
	   Putnl()
	   Define_bindings_any(BS,VD)
	 |)

'action' Define_value_bindings_prod(BINDINGS, PRODUCT_TYPE, VALUE_DEFINITION)

  'rule' Define_value_bindings_prod(list(B,nil),list(T,nil),VD):
	 Define_value_binding2(B,T,VD)

  'rule' Define_value_bindings_prod(list(B,BS),list(T,PR),VD):
	 Define_value_binding2(B,T,VD)
	 Define_value_bindings_prod(BS,PR,VD)

'action' Define_bindings_any(BINDINGS, VALUE_DEFINITION)

  'rule' Define_bindings_any(list(B,BS),VD):
	 Define_binding_any(B,VD)
	 Define_bindings_any(BS,VD)

  'rule' Define_bindings_any(nil,VD):

'action' Define_binding_any(BINDING, VALUE_DEFINITION)

  'rule' Define_binding_any(single(P,Id_or_op),VD):
	 Define_id_or_op(P, Id_or_op, any, VD -> _)

  'rule' Define_binding_any(product(P,BS),VD):
	 Define_bindings_any(BS,VD)

'action' Define_id_or_op(POS, ID_OR_OP, TYPE_EXPR, VALUE_DEFINITION -> OPT_VALUE_ID)

  'rule' Define_id_or_op(P, Id_or_op, T, VD -> nil):
  ...
  'rule' Define_id_or_op(P, Id_or_op, T, VD -> value_id(I)):
...
'action' Find_id_or_op(POS, ID_OR_OP, CLASS_ENV -> FOUND)
  ...
  'rule' Find_id_or_op(P, Id, basic_env(TYP, list(VE, VES), VAR, CH, ME, _, _, _, _, _, _, ADS) -> Found)
	 Adapt(Id, ADS -> id(Id1))
	 (|
	   where(Id1 -> id_op(Id2))
-- debug
...
'action' Define_value_from_type_def(TYPE_DEF)
  ...
  'rule' Define_value_from_type_def(abbrev(P, Id, _)):
	 ...
	 string_to_id(S1 -> Id1)
	 Define_id_or_op(P, id_op(Id1), fun(T1,total,result(bool,nil,nil,nil,nil)), no_def -> value_id(I1))
	 I'Qualifier -> Q
	 I1'Qualifier <- Q
  ...
  'rule' Define_value_from_type_def(record(P, Id, COMPS)):
	 ...
	 |)
	 Define_id_or_op(P, id_op(Id1), fun(D,total,result(T,nil,nil,nil,nil)), no_def -> value_id(I1))
...
'action' Define_value_variant(TYPE_EXPR, FUNCTION_ARROW, VARIANT, CONSTANT_CONSTRUCTOR -> CONSTANT_CONSTRUCTOR)
  ...
  'rule' Define_value_variant(T, ARR, record(P, Cons, nil), Ccid -> Ccid1):
	 (|
	   ...
	   Define_id_or_op(P1, Id_or_op, T, no_def -> value_id(I))
  ...
  'rule' Define_value_variant(T, ARR, record(P, Cons, Comps), Ccid -> Ccid):
	 ...
	 (|
	   ...
	   Define_id_or_op(P1, Id, fun(D,total,result(T,nil,nil,nil,nil)), no_def -> Oid)
	 ||
...
'action' Define_value_component(TYPE_EXPR, OPT_VALUE_ID, FUNCTION_ARROW, COMPONENT, TYPE_EXPR, INT, Value_ids, CONSTANT_CONSTRUCTOR)
  ...
  'rule' Define_value_component(T, Oid, ARR, component(P, D, _, R), T1, J, Ids, Ccid):
	 [|
	   ...
	   |)
	   Define_id_or_op(P1, Id_or_op, fun(T, ARR, result(T1,nil,nil,nil,nil)), no_def -> ODid)
	   [|
	     where(Oid -> value_id(I))
	 ...
	 [|
	   ...
	   Define_id_or_op(P1, Id_or_op, fun(product(list(T1,list(T,nil))), ARR, result(T,nil,nil,nil,nil)), no_def -> ORid)
	   [|
	     where(Oid -> value_id(I))
...
'action' Define_value_choice(Type_id, IDENT, TYPE_EXPR, CHOICE)
  ...
  'rule' Define_value_choice(I, Id, T, choice(P,N)):
	 ...
	 string_to_id(To -> Tid)
	 Define_id_or_op(P, id_op(Fid), fun(T1, total, result(T,nil,nil,nil,nil)), no_def -> value_id(FI))
	 Define_id_or_op(P, id_op(Tid), fun(T, partial, result(T1,nil,nil,nil,nil)), no_def -> value_id(TI))
...
'action' Define_value_object_def(OBJECT_DEF)
  ...
  'rule' Define_value_object_def(odef(P, Id, TS, CL)):
	 [| -- hidden or missing object reported earlier
	   ...
	   I'Env -> CE --'
	   Get_current_with(-> W)
	   Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,W,nil),C)
	   Extend_paths -> Paths
	   Extend_paths <- list(nil,Paths)
	   [|
	     ne(TS, nil)
	     Make_single_typing(TS -> TP)
	     Define_value_typing(TP, no_def)
-- debug
-- Print_typing(TP)
-- Putnl()
...
'action' Define_variable_def(VARIABLE_DEF)
  ...
  'rule' Define_variable_def(generic(P, Id, TS, T)):
     Check_types_from_typing_defined(TS)
	 Check_type_defined(T -> T1)
     Get_type_exprs_from_typings(TS -> TX)
	 Define_variable_id(P, Id, generic(T1,TX))
...
'action' Define_trans_sys_def(TRANSITION_SYS_DEF)
  ...
  'rule' Define_trans_sys_def(trans_sys_def(P, Id,Desc)):
     ...
     Define_named_trans_rule_collections(P, Desc)
     


'action' Define_named_trans_rule_collections(POS, SYSTEM_DESCRIPTION)
  'rule' Define_named_trans_rule_collections(P, desc(Input, Local, Transition)):
         where(Transition -> trans_naming(OP, Namedlist))
         Define_named_trans_rule_from_list(P, Namedlist)

  'rule' Define_named_trans_rule_collections(P, no_input(Local, Transition)):
         where(Transition -> trans_naming(OP, Namedlist))
         Define_named_trans_rule_from_list(P, Namedlist)

  'rule' Define_named_trans_rule_collections(P, with_constraint(Local, Init, Transition)):
         where(Transition -> trans_naming(OP, Namedlist))
         Define_named_trans_rule_from_list(P, Namedlist)

  'rule' Define_named_trans_rule_collections(P, as_relation(Local, Init, Rel)):
          /* do nothing, we cannot have "where" in relations. */

  'rule' Define_named_trans_rule_collections(P, _):


'action' Define_named_trans_rule_from_list(POS, NAMED_TRANS_LIST)
  'rule' Define_named_trans_rule_from_list(P, list(H,Tail)):
         Define_named_trans_rule(P, H)
         Define_named_trans_rule_from_list(P, Tail)

  'rule' Define_named_trans_rule_from_list(P, nil):


'action' Define_named_trans_rule(POS, NAMED_TRANS)
  'rule' Define_named_trans_rule(P, named_trans(I, OP)):
         /* debug 
         Putmsg("defining rule with name ")
         Print_id(I)
         Putnl() */
         Get_current_transition_rules(top -> TR)
         New_transition_rule_collection_id(P, I, OP -> Id)
         [|
           Is_defined_trans_rule_collection(I,TR) 
           Puterror(P)
           Putmsg("the collection named ")
           Print_id(I)
           Putmsg(" is already defined.")
           Putnl()
         |]
         Set_current_transition_rules(trans_rule_env(Id,TR))
         /* debug
         Print_trans_rule_env(trans_rule_env(Id,TR))*/
...
'condition' Is_defined_trans_rule_collection(IDENT, TRANS_RULE_ENV)

  'rule' Is_defined_trans_rule_collection(Id, trans_rule_env(I,TR)):
	 (|
	   I'Ident -> Id1 --'
	   eq(Id, Id1)
	 ||
	   Is_defined_trans_rule_collection(Id, TR)
	 |)
...
'action' Make_results(VALUE_EXPR -> RESULTS)
  ...
  'rule' Make_results(comp_list(P, V, list_limit(P1, B, V1, R)) -> RS):
	 ...
	 Openenv()
	 Define_value_typing(single(P1, B, T1), no_def)
	 (|
	   where(R -> restriction(P2,V2))
	   Make_results(V2 -> RS2)
	   Type_check(P2, result(bool,list(free,nil),nil,nil,nil), RS2 -> RS21)
...
-- Putnl()
  ...
  'rule' Make_results(post(P, V, post_cond(P1, R, C), PRE) -> list(result(bool,RD2,nil,nil,nil),nil)):
	 ...
	 (|
	   where(R -> result(P2, B))
	   Openenv()
	   Define_value_binding(B, T, no_def)
	   Make_results(C -> CRS)
...
--Putnl()
  ...
  'rule' Make_results(class_scope_expr(P, CL, V) -> RS):
	 ...
	 Check_value_env(CL)
	 Current -> C1
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,WITH,nil), C1)
	 Extend_paths -> Paths1
	 Extend_paths <- list(nil,Paths1)
	 Make_results(V -> RS)
	 Current <- C
  ...
  'rule' Make_results(implementation_relation(P, CL1, CL2) -> RS):
	 ...
	 I2'Env <- Oldenv1 --'
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,object_env(I1,object_env(I2,nil)),nil,nil,nil,nil,nil,WITH,nil), C)
	 Extend_paths <- list(nil,Paths)
...
-- make it pure, and of type Bool
  ...
  /* TODO: doesn't seem to work for local variables e.g. in functions (are they ever put in variable envs?) */
  'rule' Make_results(array_access(P, N, I) -> RS):
         (|
           where(N -> named_val(_, Name))
           (|
           ...
           Make_results(genvar_access(P, N, list(I,nil)) -> RS)
         ||
           /* Otherwise, it is, indeed, an array access */
           Make_results(I -> RS1)
           Make_results(N -> RS2)
  ...
  'rule' Make_results(genvar_access(P, N, list(I,Tail)) -> RS):
         Make_results(N -> RS2)
         Type_check(P, result(generic(any,any),list(free,nil), list(free,nil), list(free,nil), list(free,nil)), RS2 -> _)
         where(RS2 -> list(Res,_))
         where(Res -> result(T,_,_,_,_))
         (|
           Remove_Brackets(T -> generic(TV,type_exprs(list(TI,TTail))))
           Make_results_for_genvar_index(P, list(TI,TTail), list(I,Tail))
           Add_accesses(RS2, list(result(TV,nil,nil,nil,nil),nil) -> RS)
         ||
           where(RESULTS'nil -> RS) --'
         |)
...
'action' Make_results_for_genvar_index(POS, TYPE_EXPRS, VALUE_EXPRS)
  'rule' Make_results_for_genvar_index(P, list(ActualType,TailT), list(FoundVal,TailV)):
         Make_results_for_genvar_index(P, TailT, TailV)
         Make_results(FoundVal -> RS1)
         Type_check(P, result(ActualType,list(free,nil), list(free,nil), list(free,nil), list(free,nil)),RS1 -> _)
         

  'rule' Make_results_for_genvar_index(P, nil, list(H,_)):
         Puterror(P)
         Putmsg("Number of access arguments does not match number of types over which the generic variable is defined")
         Putnl()

  'rule' Make_results_for_genvar_index(P, list(H,_), nil):
         Puterror(P)
         Putmsg("Number of access arguments does not match number of types over which the generic variable is defined")
         Putnl()

  'rule' Make_results_for_genvar_index(P, nil, nil): 
         

'action' Make_results_for_genvar_cmd(POS, TYPE_EXPR, VALUE_EXPRS -> RESULTS)
  'rule' Make_results_for_genvar_cmd(P, Type, list(H,Tail) -> RS)
         Type_check(P, result(generic(any,any), list(free,nil), list(free,nil), list(free,nil), list(free,nil)), list(result(Type, list(free,nil), list(free,nil), list(free,nil), list(free,nil)),nil) -> _)             
        (|
           Remove_Brackets(Type -> generic(VT,type_exprs(list(IT,TailT))))
           Make_results_for_genvar_cmd2(P, list(IT,TailT), list(H,Tail))
           where(RESULTS'list(result(VT,nil,nil,nil,nil),nil) -> RS) /* TODO: is this correct? */
         ||
           where(RESULTS'nil -> RS)
         |)


'action' Make_results_for_genvar_cmd2(POS, TYPE_EXPRS, VALUE_EXPRS)
  'rule' Make_results_for_genvar_cmd2(P, list(ActualType,TailT), list(FoundVal,TailV)):
         Make_results(FoundVal -> RS1)
         Type_check(P, result(ActualType, list(free,nil), list(free,nil), list(free,nil), list(free,nil)), RS1 -> _)
         Make_results_for_genvar_cmd2(P, TailT, TailV)

  'rule' Make_results_for_genvar_cmd2(P, nil, list(H,_)):
         Puterror(P)
         Putmsg("Number of access arguments does not match number of types over which the generic variable is defined")
         Putnl()

  'rule' Make_results_for_genvar_cmd2(P, list(H,_), nil):
         Puterror(P)
         Putmsg("Number of access arguments does not match number of types over which the generic variable is defined")
         Putnl()

  'rule' Make_results_for_genvar_cmd2(P, nil, nil):      
...
'action' Make_array_expr_results(VALUE_EXPR -> RESULT)
  'rule' Make_array_expr_results(array_expr(P,Typing,Val) -> result(array(IndexType,ValType),A1,A2,A3,A4))
         ...
         Openenv()
         Define_value_typing(Typing, no_def)
...
'action' Open_let_envs(LET_DEFS -> ACCESSES, ACCESSES, ACCESSES, ACCESSES)
  ...
  'rule' Open_let_envs(list(explicit(P,binding(P1,B),V), DEFS) -> RD,WR,IN,OUT):
	 ...
	 Openenv()
	 Define_value_typing(single(P, B, T), no_def)
...
'action' Define_pattern(POS, TYPE_EXPR, PATTERN)
  ...
  'rule' Define_pattern(P, T, id_pattern(P1, I)):
	 Define_id_or_op(P1, I, T, no_def -> _)
...
'action' Lookup_env_value_name(NAME, CLASS_ENV, OWNENV, OWNCLASS -> Value_ids)
  ...
  'rule' Lookup_env_value_name(N, CE, Ownenv, Ownclass -> Ids):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,VES,_,_,_,_,_,_,_,_,Objs,_))
...
'action' Lookup_env_value_name2(ID_OR_OP, CLASS_ENV -> Value_ids)
  ...
  'rule' Lookup_env_value_name2(Id_or_op, CE -> Ids):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,VES,_,_,_,_,_,_,_,_,_,_))
...
'action' Lookup_env_variable_name(NAME, CLASS_ENV, OWNENV, OWNCLASS  -> OPT_VARIABLE_ID)
  ...
  'rule' Lookup_env_variable_name(N, CE, Ownenv, Ownclass -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,VES,VARS,_,_,_,_,_,_,_,Objs,_))
	     -- only use withs if own environment
	     (|
	       eq(Ownenv,ownenv)
...
'action' Lookup_env_variable_name2(IDENT, CLASS_ENV  -> OPT_VARIABLE_ID)
  ...
  'rule' Lookup_env_variable_name2(Id, CE -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,VES,VARS,_,_,_,_,_,_,_,_,_))
...
'action' Lookup_env_channel_name(NAME, CLASS_ENV, OWNENV, OWNCLASS  -> OPT_CHANNEL_ID)
  ...
  'rule' Lookup_env_channel_name(N, CE, Ownenv, Ownclass -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,_,_,CHS,_,_,_,_,_,_,Objs,_))
	     -- only use withs if own environment
	     (|
	       eq(Ownenv,ownenv)
...
'action' Lookup_env_channel_name2(IDENT, CLASS_ENV  -> OPT_CHANNEL_ID)
  ...
  'rule' Lookup_env_channel_name2(Id, CE -> I):
	 ...
	 (|
	   ne(Oid,nil)
	   (|
	     where(CE -> basic_env(_,_,_,CHS,_,_,_,_,_,_,_,_))
...
'action' Type_check_variable_def(VARIABLE_DEF)
  ...
  'rule' Type_check_variable_def(generic(P, Id, TS, T)):
     Type_check_type(T)
     Type_check_typings(TS)
     Get_current_variables(-> VARS)
	 Lookup_env_variable_id(Id, nil, VARS -> variable_id(I))
	 I'Type -> T1 --'
	 Check_self_read_or_write_access(T1, I)
     /* The types from the typing must be finite for generic variables.
     Only then can the generic variable meaningfully be unfolded to separate variables. */
     Check_types_from_typing_finite(TS)
     -- TODO: more?
     /* When initialisations of generic variables are possible, 
        we should add something like the above for single variables, 
        where we check that there is an init, and then Make_results, 
        which... 
        and then type check the resulsts.  */
...
'action' Type_check_value_def(VALUE_DEF)
  ...
  'rule' Type_check_value_def(gen_val(P, Id, TS, TE)):
	 Type_check_typings(TS)
     /* The types from the typing must be finite for generic values.
     Only then can the generic value meaningfully be unfolded to separate values. */
     Check_types_from_typing_finite(TS)
     Type_check_type(TE)
...
'action' Result_type(POS, TYPE_EXPR, FORMAL_FUNCTION_PARAMETERS -> RESULT)
  ...
  'rule' Result_type(P, fun(T, A, R), list(form_parm(P2,BS), nil) -> R):
	 Define_value_bindings(P, BS, T, no_def)
...
-----------------
     ...
     Get_current_transition_rules(top -> TR)
	 Current -> C
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,TR,nil,WITH,nil), C)
	 Extend_paths -> Paths
	 Extend_paths <- list(nil,Paths)
...
----------------------------
  ...
  'rule' Type_check_trans_sys_def(trans_sys_def(P,_,Defs)):
	 where(Defs -> with_constraint(Local, Constraint, Transitions))
-----------------
	 Get_current_with(-> WITH)
     Get_current_transition_rules(top -> TR) 
	 Current -> C
	 ...
	 Make_value_env(basic(DS))
	 Check_value_env(basic(DS))
     Type_check_constraint(P, Constraint)
	 Type_check_transitions(P, Transitions)
----------------------------
	 Current -> current_env(CE, C1)
	 Current <- C1
  ...
  'rule' Type_check_trans_sys_def(trans_sys_def(P,_,Defs)):
	 where(Defs -> as_relation(Local, Constraint, Relation))
-----------------
	 Get_current_with(-> WITH)
     Get_current_transition_rules(top -> TR) 
	 Current -> C
	 ...
	 Check_value_env(basic(DS))
     Type_check_constraint(P, Constraint)
	 Type_check_transrel(P, Relation)
----------------------------
	 Current -> current_env(CE, C1)
	 Current <- C1
	 Extend_paths <- Paths
...
'action' Type_check_constraint(POS, CONSTRAINT)
  'rule' Type_check_constraint(P, constraint(PP, Cs)):
         Type_check_conjunctions(PP, Cs)

'action' Type_check_conjunctions(POS, CONJUNCTION_LIST)

  'rule' Type_check_conjunctions(P, list(Val,Tail)):
         Make_results(Val -> Results)
         Get_reads(Results -> RD)
         /* Check the read accesses for primed variables. These are not allowed in the initialisation constraint. */
         Check_accesses_for_primes(P,RD)
         /* Check that the constraint is indeed boolean */
         Type_check(P, result(bool,list(free,nil),list(free,nil),list(free,nil),list(free,nil)), Results -> _)
         Type_check_conjunctions(P,Tail)

  'rule' Type_check_conjunctions(_, nil)

'action' Check_accesses_for_primes(POS, ACCESSES)
  'rule' Check_accesses_for_primes(P, list(Acc,Rest)):
         Check_accesses_for_primes(P,Rest)
         [|
           where(Acc -> variable(_, I, _))
           I'Ident -> Id  --'
           id_to_string(Id -> Str)
	       Is_primed(Str)
           Puterror(P)
           Putmsg("Reference to primed variable (")
           Print_id(Id)
           Putmsg(") is not allowed here.")
           Putnl()
         |]

  'rule' Check_accesses_for_primes(P, nil):

'action' Type_check_transrel(POS, TRANSREL)
  'rule' Type_check_transrel(P, transrel(PP, Val)):
         Make_results(Val -> Results)
         /* Check that the transrel is indeed boolean */
         Type_check(PP, result(bool,list(free,nil),list(free,nil),list(free,nil),list(free,nil)), Results -> _)           
...
'action' Type_check_transitions(POS, TRANSITION_OPERATOR)
  ...
  'rule' Type_check_transitions(P, trans_naming(OP,NAMEDLIST)):
         Type_check_transitions(P, OP) /* check the sytem comp */
         Type_check_named_trans_list(P, NAMEDLIST) /* check the "where" list */

  'rule' Type_check_transitions(P, trans_ident(IDENT)):
         /* Check that the identifier is defined in the environment */
         Get_current_transition_rules(all -> TR_env)
         Get_Transition_Rule(IDENT, TR_env -> OptTRId)
         [|
          where(OptTRId -> nil) /* cannot find a named rule collection with this name */
          Puterror(P)
          Putmsg("A transition rule with name ")
          Print_id(IDENT)
          Putmsg(" is referenced, but has not been defined")
          Putnl()
         |]        

'action' Type_check_named_trans_list(POS, NAMED_TRANS_LIST)
  'rule' Type_check_named_trans_list(P, list(H, Tail)):
         Check_for_circularity(P, list(H, Tail), list(H, Tail)) /* check for circular references in module bodies */
         Type_check_named_trans(P, H)
         Type_check_named_trans_list(P, Tail)

  'rule' Type_check_named_trans_list(P, nil):

'action' Type_check_named_trans(POS, NAMED_TRANS) 
  'rule' Type_check_named_trans(P, named_trans(Name, OP)):
         /* At this point, it has been already checked that the name is not used multiple times. */
         Type_check_transitions(P, OP)


'action' Check_for_circularity(POS, NAMED_TRANS_LIST, NAMED_TRANS_LIST)
  'rule' Check_for_circularity(P, list(H,Tail), AllTrans):
         Check_for_circularity_in_named_trans(P,H,AllTrans)
         Check_for_circularity(P,Tail,AllTrans)

  'rule' Check_for_circularity(P,nil,_):

'action' Check_for_circularity_in_named_trans(POS, NAMED_TRANS, NAMED_TRANS_LIST)
  'rule' Check_for_circularity_in_named_trans(P, named_trans(Name,OP), NTL):
         Check_for_circularity_in_transop(P, list(Name,nil), OP, NTL)

'action' Check_for_circularity_in_transop(POS, NAME_LIST, TRANSITION_OPERATOR, NAMED_TRANS_LIST)
  'rule' Check_for_circularity_in_transop(P, List, equal_priority(L,R,G), NTL):
         Check_for_circularity_in_transop(P, List, L, NTL) 
         Check_for_circularity_in_transop(P, List, R, NTL)
         
  'rule' Check_for_circularity_in_transop(P, List, greater_priority(L,R,G), NTL):
         Check_for_circularity_in_transop(P, List, L, NTL) 
         Check_for_circularity_in_transop(P, List, R, NTL)

  'rule' Check_for_circularity_in_transop(P, List, bracketed(O,G), NTL):
         Check_for_circularity_in_transop(P, List, O, NTL) 

  'rule' Check_for_circularity_in_transop(P, List, guarded_command(CM,G), NTL):
         -- do nothing as guarded commands cannot refer to transition collection identifiers

  'rule' Check_for_circularity_in_transop(P, List, trans_naming(OP,NAMEDLIST), NTL): 
         -- TODO we do not want this (nested "where"s) - how to handle?

  'rule' Check_for_circularity_in_transop(P, list(H, Tail), trans_ident(I), NTL):
         (|
           Is_ident_in_list(P, list(H, Tail), I) -- check if identifier is in list
           Puterror(P)
	       Putmsg("Reference to named collection: ") 
           Print_id(I) 
           Putmsg(" causes circular references.")
           Putnl()
         ||
           Get_def_for_I(P, list(I, list(H, Tail)), I, NTL, NTL) -- add I to list and recurse        
         |)    

'action' Get_def_for_I(POS, NAME_LIST, IDENT, NAMED_TRANS_LIST, NAMED_TRANS_LIST)
  'rule' Get_def_for_I(P, Namelist, I, list(H, Tail), NTL):
         Get_def_for_I_2(P, Namelist, I, H, list(H, Tail))
         Get_def_for_I(P, Namelist, I, Tail, NTL)

  'rule' Get_def_for_I(P, Namelist, I, nil, NTL):

'action' Get_def_for_I_2(POS, NAME_LIST, IDENT, NAMED_TRANS, NAMED_TRANS_LIST)
  'rule' Get_def_for_I_2(P, Namelist, I, named_trans(Name, OP), NTL):
         [| 
           /* found the def for the recently added I, now check for circularities in that def */
           eq(I, Name)
           Check_for_circularity_in_transop(P, Namelist, OP, NTL)
         |]

'condition' Is_ident_in_list(POS, NAME_LIST, IDENT)
     'rule' Is_ident_in_list(P, list(H, Tail), I):
            (| 
              eq(H,I)
            ||
             Is_ident_in_list(P, Tail, I)
            |)
...
'action' Type_check_transitions_else_not_allowed(POS, TRANSITION_OPERATOR)
  ...
  'rule' Type_check_transitions_else_not_allowed(P, trans_naming(OP,NAMEDLIST)): 
         Type_check_transitions_else_not_allowed(P, OP)
         Type_check_named_transitions_list_else_not_allowed(P, NAMEDLIST)

  'rule' Type_check_transitions_else_not_allowed(P, trans_ident(I)):
         Type_check_transitions(P, trans_ident(I)) /* For getting type checking of references to named collections of rules*/

'action' Type_check_named_transitions_list_else_not_allowed(POS, NAMED_TRANS_LIST)
  'rule' Type_check_named_transitions_list_else_not_allowed(P, list(H, Tail)):
         Type_check_named_transition_else_not_allowed(P, H)
         Type_check_named_transitions_list_else_not_allowed(P, Tail)

  'rule' Type_check_named_transitions_list_else_not_allowed(P, nil):

'action' Type_check_named_transition_else_not_allowed(POS, NAMED_TRANS)
  'rule' Type_check_named_transition_else_not_allowed(P, named_trans(I, OP)):
         Type_check_transitions_else_not_allowed(P, OP)
...
--------------	 
         (|
           ...
           (|
             ...
             Match(Type,down,generic(any,any))
             where(In -> list(H,Tail))
             /* Always only the first [ ] for the generic variable access.
             Check that the index fits with the generic variables index type. */
             Make_results(genvar_access(P, named_val(P,N), list(H, nil)) -> GenRes)
             /* If there are more indices, check the array access consisting of the indexed generic 
                variable and an array access, i.e. make sure that the instance of the generic variable 
                corresponds to the array type. */
             [|
               ne(Tail, nil)
               where(GenRes -> list(R1,_))
               where(R1 -> result(T,_,_,_,_))
               Make_results_for_array_occ(T, Tail, P -> RSB)
               Make_results(V -> RSA)
               where(RSB -> list(R2,_))
               where(R2 -> result(T2,_,_,_,_))
               Type_check(P, result(T2,list(free,nil),list(free,nil),list(free,nil),list(free,nil)), RSA -> RSA1)
             |]
             (|
               Is_primed_Name(N)
             ||
	         ...
	         |)
           ||
             /* Otherwise, it is, indeed, an array access (without any generic var access involved). */ 
             Make_results(V -> RSA)
...
--	 Make_unique_result(P, E -> _)
  ...
  'rule' Type_check_commands(list(genvar_cmd(P,N,In,V),Cmds))
--------------
        (|
	      Lookup_variable_name(N -> variable_id(I))
          I'Type -> Type --'
          Make_results(V -> RSA)
          Make_results_for_genvar_cmd(P, Type, In -> RSB)
          (|
             where(RSB -> list(R1,_))
             where(R1 -> result(T,_,_,_,_))
	     Type_check(P, result(T,list(free,nil),list(free,nil),list(free,nil),list(free,nil)), RSA -> RSA1)           
...
'action' Type_check_property_def(PROPERTY_DECL)
  ...
  'rule' Type_check_property_def(property_def(Pos, Name, TS, LTL_prop)):
	 ...
	 TSId'System -> TS_Decls --'
	  ...
	  ---------------------------------------------------------
	 where(TS_Decls -> with_constraint(Local, Constraint, Transitions))
	 -- Puth the valid variables (from the TS) in the current new env:
	 Get_current_with(-> WITH)
	 Current -> C
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,WITH,nil), C)
...
-- Utilities
-----------------------------------------------------------------
'action' Get_type_exprs_from_typings(TYPINGS -> TYPE_EXPRS_GEN)
  'rule' Get_type_exprs_from_typings(list(H,Tail) -> type_exprs(list(Hx,Tailx))):
         Get_type_expr_from_typing(H -> Hx)
         Get_type_exprs_from_typings(Tail -> type_exprs(Tailx))

  'rule' Get_type_exprs_from_typings(nil -> type_exprs(nil)):

'action' Get_type_expr_from_typing(TYPING -> TYPE_EXPR)
  'rule' Get_type_expr_from_typing(single(P,B,T) -> T):

  'rule' Get_type_expr_from_typing(multiple(P,B,T) -> T):
...
'action' Generate_Primes(VARIABLE_DEFS -> VARIABLE_DEFS)
  ...
  'rule' Generate_Primes(list(H, Rest) -> list(H_prime, Rest_prime))
     ...
     ||
		where(H -> VARIABLE_DEF'generic(Pos, Ident, Typings, Type)) --'
		id_to_string(Ident -> IdStr)
		IncreaseCol(Pos -> NewPos)
		Concatenate(IdStr,"'" -> NewIdStr)
		string_to_id(NewIdStr -> NewIdent)
		where(VARIABLE_DEF'generic(NewPos, NewIdent, Typings, Type) -> H_prime) --'
	 ||
