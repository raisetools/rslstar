...
--       Print_restriction(R)
  ...
  'rule' Print_type(generic(T,TS)):
         Putmsg("generic with type ")
         Print_type(T)
         Putmsg(" over types ")
         Print_types_for_gen(TS)
...
-- special code for printing U, R, and W in LTL as infix
  ...
  'rule' Print_expr(genvar_access(P, N, I)):
         Print_expr(N)
         Putmsg("[")
         Print_exprs(I)
         Putmsg("]")
...
'action' Expr_precedence(VALUE_EXPR -> INT)
  ...
  'rule' Expr_precedence(genvar_access(_,_,_) -> 0)

  'rule' Expr_precedence(array_expr(_,_,_) -> 0)

  'rule' Expr_precedence(array_assignment(_,_,_,_) -> 11):
...
'action' Print_types_for_gen(TYPE_EXPRS_GEN)

  'rule' Print_types_for_gen(type_exprs(list(H,nil))):
         Print_type(H)

  'rule' Print_types_for_gen(type_exprs(list(H,T))):
         Print_type(H)
         Putmsg(" and ")
         Putnl()
         Print_types_for_gen(type_exprs(T))

  'rule' Print_types_for_gen(type_exprs(nil)):
...
'action' Print_trans_rule_env(TRANS_RULE_ENV)

  'rule' Print_trans_rule_env(trans_rule_env(Id, TR)):
	 Putmsg("[")
     Id'Ident -> Name
     Print_id(Name)
	 Putmsg("]")
	 [|
	   ne(TR, nil)
	   Putmsg(",")
	   Print_trans_rule_env(TR)
	 |]

  'rule' Print_trans_rule_env(nil):


'action' Print_variable_env(VARIABLE_ENV)

  'rule' Print_variable_env(variable_env(Id, VE)):
	 Putmsg("var:")
     Id'Ident -> Name
     Print_id(Name)
	 Putmsg(" ")
	 [|
	   ne(VE, nil)
	   Putmsg(",")
	   Print_variable_env(VE)
	 |]

  'rule' Print_variable_env(nil):
