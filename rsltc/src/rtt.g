...
'use' ast ext env types objects values print
...
'action' gen_sym_table_decls(DECLS)
  ...
  'rule' gen_sym_table_decls(list(type_decl(P, TDs), Ds)):
         /* Creating the type environment is necessary for lookups of array index types, so the dimensions
            can be generated. */
         Current -> C
         Current <- current_env(basic_env(nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil), C)
         Extend_paths -> Paths
         Extend_paths <- list(nil,Paths)
         where(DECLS'list(type_decl(P, TDs), nil) -> DS) --'
...
'action' gen_value_expr(VALUE_EXPR)
  ...
  'rule' gen_value_expr(val_infix(_, Left, isin, Right)):
         gen_value_expr(Left)
         WriteFile(" isin ")
         gen_value_expr(Right)
  ...
  /* Mine. */
  'rule' gen_value_expr(var_occ(P, I, Q)):
         WriteFile("var_occ")

  'rule' gen_value_expr(ass_occ(P, I, Q, V)):
         WriteFile("ass_occ")

  'rule' gen_value_expr(Other):
         Print_expr(Other)
...
'action' gen_type_expr(TYPE_EXPR)
  ...
  'rule' gen_type_expr(array(D,R)):
         gen_type_expr(R)
         WriteFile("[")
         gen_array_dimension(D)
         WriteFile("]")


'action' gen_array_dimension(TYPE_EXPR)

  'rule' gen_array_dimension(subtype(T,restriction(P,VE))):
         /* Before beginning the generation of the intermediate file, it has already been checked 
            that the array index type is one of the expected types. */
         (|
           ...
           String_to_int(SId -> I)
           where(I + 1 -> NumDimen)
           Int_to_string(NumDimen -> Dimen)
         ||
           where(VE -> ax_infix(P1,VEL,CONN,VER,_))
           where(VER -> val_infix(_,VEL2,OP2,VER2))
           where(VER2 -> literal_expr(_,int(Id)))
           id_to_string(Id -> Dimen)
         |)
         WriteFile(Dimen)
                
  'rule' gen_array_dimension(named_type(Name)):
         where(Name -> name(P,id_op(Id)))
         Lookup_type_id(P, Id -> type_id(I))
         I'Type -> TE --'
         where(TE -> abbrev(T))
         gen_array_dimension(T)
