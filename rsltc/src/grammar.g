...
'use' ast ext env print types values objects cc pp cpp sml
      pvs pvs_aux sal sal_global_vars fdr rtt unfold
...
'root'
	...
	SAL_Translate
      ...
      |]
      [|
       /* TODO: Get back to normal! This is a hack to make the unfolding steps + translation to SAL in one go. */
        UnfoldTransCollectionNamesWanted()
        /*Make_env(M, G)*/
        unfold_to_rsl(M -> UM)
        Putmsg("First pass (unfold named collections of transition rules) completed...")
        Putnl()
        Putmsg("Setting parameter for second pass...")
        SetUnfoldGenRulesProps()
        Putnl()
        [|
         UnfoldGenericRulesPropsWanted()
         Putmsg("Successfully set...")
         Putnl()
        |]
        unfold_to_rsl(UM -> UUM)
        Putmsg("Second pass (unfold generic rules and properties) completed...")
        Putnl()
        Putmsg("Setting parameter for third pass...")
        SetUnfoldGenVarRefs()
        Putnl()
        [|
         ...
         Putnl()
        |]
        unfold_to_rsl(UUM -> UUUM)
        Putmsg("Third pass (unfold generic variable names) completed...")
        Putnl()
        /*SetSAL()
        SAL_init
        SAL_Translate*/
      ...
      [|
        ...
        Putmsg("Transforming to transition relation...")
        Putnl()
        unfold_to_transrel(M -> UM)
        Putmsg("Finished transforming...")
        Putnl()
      |]
    |)
...
Putnl()
	   ...
	   Parameters <- nil
	   Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,nil,nil),nil)
	   Extend_paths <- list(nil,nil)
...
Putnl()
	   ...
	   Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,nil,nil),C)
	   [|
	     ne(TS, nil)
...
Putnl()
	 ...
	 Extend_paths <- list(nil,nil)
	 Current <-
	 current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,nil,nil),nil)
...
Putnl()
	 ...
	 Current <- current_env(basic_env(nil,list(nil,nil),nil,nil,nil,nil,nil,nil,nil,nil,nil,nil),C1)
	 [|
	   ne(TS, nil)
	   Make_single_typing(TS -> TP)
	   Define_value_typing(TP, no_def)
...
'nonterm' gen_value_def(-> VALUE_DEF)

   'rule' gen_value_def(-> gen_val(P, Id, TS, T))
      id(-> Id)
      "["
      typing_list(-> TS)
      "]"
...
'nonterm' gen_variable_def(-> VARIABLE_DEF)

   'rule' gen_variable_def(-> generic(P, Id, TS, T))
      id(-> Id)
      "["
      typing_list(-> TS)
      "]"
...
'nonterm' sys_description(-> SYSTEM_DESCRIPTION)
  ...
  'rule' sys_description(-> with_constraint(Local,
                 Constraint,
				 Transitions)):
       trans_sys_locals(-> Local)
       trans_sys_constraint(-> Constraint)
	   trans_sys_trans(-> Transitions)

  'rule' sys_description(-> as_relation(Local,
                 Constraint,
				 Relation)):
       trans_sys_locals(-> Local)
       trans_sys_constraint(-> Constraint)
	   trans_sys_transrel(-> Relation)
...
'nonterm' trans_sys_constraint(-> CONSTRAINT)

   'rule' trans_sys_constraint(-> constraint(P,Constraints)):
      "init_constraint" @(-> P)
        constraint_list(-> Constraints)

'nonterm' constraint_list(-> CONJUNCTION_LIST)

  'rule' constraint_list(-> list(H,nil)):
         value_infix_expr_pr6(-> H)
...
'nonterm' trans_sys_transrel(-> TRANSREL)
      
   'rule' trans_sys_transrel(-> transrel(P,Rel))
      "transition_relation" @(-> P)
      value_expr(-> Rel)
      "end"
...
'nonterm' transitionsoperator(-> TRANSITION_OPERATOR, EXTRA_GUARD)
  'rule' transitionsoperator(-> trans_naming(TRANSOP, NAMEDLIST), EXTRA):
          transitionsoperator(-> TRANSOP, EXTRA)
          "where"
          transitions_named_list(-> NAMEDLIST)
  ...
  'rule' transitionsoperator(-> guarded_command(H, EXTRA), EXTRA):
      ...
      'nonterm' transitions_named_list(-> NAMED_TRANS_LIST)
   'rule' transitions_named_list(-> list(H,T)):
            transition_named(-> H)
            ","
            transitions_named_list(-> T)

   'rule' transitions_named_list(-> list(H,nil)):
            transition_named(-> H)

'nonterm' transition_named(-> NAMED_TRANS)
   'rule' transition_named(-> named_trans(N,TRANS)):
            "["
            id(-> N)
            "]"
            "="
            transitionsoperator_bracketed(-> TRANS, _) 
...
'nonterm' transitionsoperator_greater(-> TRANSITION_OPERATOR, EXTRA_GUARD)
  ...
  'rule' transitionsoperator_greater(-> trans_ident(I),nil):
      id(->I)
...
'nonterm' transitionsoperator_equal(-> TRANSITION_OPERATOR, EXTRA_GUARD)
  ...
  'rule' transitionsoperator_equal(-> trans_ident(I),nil):
      id(->I)
...
'nonterm' sal_update_expr(-> COMMAND)
  ...
  'rule' sal_update_expr(-> genvar_cmd(P,Name, IndexExpr, ValueExpr))
         name(-> Name) @(-> P)
         genvar_accesses(-> IndexExpr)
         "="
         value_expr(-> ValueExpr)      
...
'nonterm' genvar_access_expr(-> VALUE_EXPR)
  'rule' genvar_access_expr(-> genvar_access(P, named_val(PN,N), I))
         name(-> N) @(-> PN)
         "["
         genvar_accesses2(-> I)      
         "]"   
  ...
  'rule' genvar_access_expr(-> genvar_access(P, N, I))
         value_expr_pr0(-> N)
         "["
         genvar_accesses2(-> I)
         "]"
...
'nonterm' genvar_accesses(-> VALUE_EXPRS)
  'rule' genvar_accesses(-> V)
         "["
         genvar_accesses2(-> V)
         "]"
      
'nonterm' genvar_accesses2(-> VALUE_EXPRS)      
  'rule' genvar_accesses2(-> list(H, nil))
         value_expr(-> H)

  'rule' genvar_accesses2(-> list(H,T))
         value_expr(-> H)
         ","
         genvar_accesses2(-> T)      
